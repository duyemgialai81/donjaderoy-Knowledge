...
function readExpiringCache<T>(
  cache: ExpiringMemoryCache<T>,
  key: string,
  ttlMs: number
): T | null {
  const entry = cache.get(key);
...
// ==================== PURE HELPERS (module-level, no hoisting issues) ====================
// Defined at module scope so they are never part of the TDZ that causes the "ht" error.
const toId = (value: unknown): string =>
  value === undefined || value === null ? "" : String(value);

const parseRealtimePayload = (data: any): any => {
  if (!data) return null;
  if (typeof data === "object") return data;
  const s = String(data).trim();
  if (!s || s === "undefined" || s === "null") return null;
...
const dedupConversations = (list: ConversationItem[]): ConversationItem[] => {
  const map = new Map<string, ConversationItem>();
  list.forEach((c) => {
    const key = c.targetUserId ? String(c.targetUserId) : String(c.id);
    if (!map.has(key)) map.set(key, c);
...
export default function MessagesPage({ currentUser }: MessagesPageProps) {
  ...
  // ── Derive currentUserId FIRST so every hook below can reference it safely ──
  const currentUserId = toId(currentUser?.id);
  ...
  // Keep refs in sync with state (these useEffects have no deps that could be TDZ)
  useEffect(() => {
  ...
  // Cache conversations (currentUserId is safe here — derived before any hook)
  useEffect(() => {
    if (!currentUserId || conversations.length === 0) return;
    writeExpiringCache(chatListMemoryCache, `chat-list:${currentUserId}`, conversations);
  }, [conversations, currentUserId]);
  ...
  useEffect(() => {
    if (
      !currentUserId ||
      !selectedChatId ||
      selectedChatId.startsWith("new_") ||
      messages.length === 0
    )
      return;
    writeExpiringCache(
      messageHistoryMemoryCache,
      `messages:${currentUserId}:${selectedChatId}`,
      messages
    );
  }, [messages, selectedChatId, currentUserId]);

  // ==================== HELPERS ====================
  const getAvatarUrl = useCallback((url?: string, id?: string) => {
    return normalizeAvatarUrl(url, id || "default");
  }, []);

  const getConversationPeerId = useCallback(
    (conversation?: Pick<ConversationItem, "id" | "targetUserId"> | null): string => {
      if (!conversation) return "";
      const targetUserId = toId(conversation.targetUserId);
      if (targetUserId) return targetUserId;
      const conversationId = toId(conversation.id);
  ...
  const getEventPeerId = useCallback(
    (senderId?: unknown, receiverId?: unknown): string => {
      const s = toId(senderId);
      const r = toId(receiverId);
      if (s && s === currentUserId) return r;
      if (r && r === currentUserId) return s;
      return s || r;
    },
  ...
  const doesEventMatchSelectedChat = useCallback(
    (conversationId?: unknown, senderId?: unknown, receiverId?: unknown): boolean => {
      const selectedId = toId(selectedChatIdRef.current);
      if (!selectedId) return false;
      if (toId(conversationId) === selectedId) return true;
      const sel = conversationsRef.current.find((c) => toId(c.id) === selectedId);
      const selPeerId = sel
        ? getConversationPeerId(sel)
    ...
    [getConversationPeerId, getEventPeerId]
  );
  ...
  const selectedChatBackground =
    CHAT_BACKGROUNDS.find((item) => item.id === chatBackgroundId) || CHAT_BACKGROUNDS[0];
  const normalizedCustomChatBackground = customChatBackground
    ? normalizeAssetUrl(customChatBackground) || customChatBackground
    : "";
  const selectedChatBackgroundValue = normalizedCustomChatBackground
    ? `linear-gradient(rgba(248,250,252,0.84), rgba(248,250,252,0.84)), url("${normalizedCustomChatBackground}") center/cover`
    : selectedChatBackground.value;
  ...
  // ==================== BACKGROUND HELPERS ====================
  const updateChatBackground = useCallback((id: string) => {
    setChatBackgroundId(id);
    setCustomChatBackground("");
    try {
      localStorage.setItem(CHAT_BACKGROUND_STORAGE_KEY, id);
  ...
  const updateCustomChatBackground = useCallback((url: string) => {
    const normalizedUrl = normalizeAssetUrl(url) || url;
    setCustomChatBackground(normalizedUrl);
    try {
      localStorage.setItem(CUSTOM_CHAT_BACKGROUND_STORAGE_KEY, normalizedUrl);
  ...
  const sendCallSignal = useCallback(
    (
      ...
      if (!stompClientRef.current?.connected) return;
      let convId = selectedChatIdRef.current?.startsWith("new_")
        ? ""
        : selectedChatIdRef.current || "";
      if (!convId && (type === "end" || type === "reject")) {
        const found = conversationsRef.current.find(
          (c) => getConversationPeerId(c) === targetId
        );
        if (found && !found.id.startsWith("new_")) convId = found.id;
      }
      stompClientRef.current.publish({
  ...
  const logCallStatusToChat = useCallback(
    (session: CallSession, reason: "ended" | "missed" | "rejected") => {
      if (!stompClientRef.current?.connected) return;
      const logText =
        reason === "rejected"
          ? session.mode === "video"
            ? "📹 Đã từ chối cuộc gọi video"
            : "📞 Đã từ chối cuộc gọi thoại"
          : session.elapsedSeconds === 0 || reason === "missed"
          ? session.mode === "video"
            ? "📹 Cuộc gọi video nhỡ"
            : "📞 Cuộc gọi thoại nhỡ"
      ...
      if (!convId) {
        const found = conversationsRef.current.find(
          (c) => getConversationPeerId(c) === session.peerId
        );
        if (found && !found.id.startsWith("new_")) convId = found.id;
      }
      stompClientRef.current.publish({
  ...
  // closeCall declared early with useCallback so it can be referenced in other callbacks below
  const closeCall = useCallback(
    (isLocal = true, isReject = false) => {
      if (acceptCallTimeoutRef.current) clearTimeout(acceptCallTimeoutRef.current);
      const s = callSessionRef.current;
      if (s && isLocal) {
        if (isReject) {
          logCallStatusToChat(s, "rejected");
          sendCallSignal("reject", null, s.peerId, s.id, s.mode);
        } else if (s.status === "incoming") {
          logCallStatusToChat(s, "rejected");
          sendCallSignal("reject", null, s.peerId, s.id, s.mode);
        } else {
          logCallStatusToChat(s, s.elapsedSeconds === 0 ? "missed" : "ended");
          sendCallSignal("end", null, s.peerId, s.id, s.mode);
        }
      }
    ...
    [logCallStatusToChat, sendCallSignal]
  );

  const replaceLocalVideoTrack = useCallback((track: MediaStreamTrack) => {
  ...
  const createPeerConnection = useCallback(
    (targetId: string, callId: string, callMode: CallMode) => {
      const pc = new RTCPeerConnection(iceServers);
      pc.onicecandidate = (e) => {
        if (e.candidate)
          sendCallSignal(
  ...
  const stopScreenShare = useCallback(async () => {
    ...
    try {
      const cameraStream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: s.cameraFacing },
        audio: false,
      });
      cameraStreamRef.current?.getVideoTracks().forEach((track) => track.stop());
      cameraStreamRef.current = cameraStream;
      ...
      if (cameraTrack) replaceLocalVideoTrack(cameraTrack);
      setCallSession((prev) =>
        prev ? { ...prev, isScreenSharing: false, isCameraOff: false } : prev
      );
    } catch {
      toast.error("Không thể bật lại camera.");
      setCallSession((prev) =>
        prev ? { ...prev, isScreenSharing: false, isCameraOff: true } : prev
      );
    }
  }, [replaceLocalVideoTrack]);

  const getCallMediaStream = useCallback(async (mode: CallMode): Promise<MediaStream> => {
    if (mode === "audio")
  ...
  const applyIncomingCallEvent = useCallback(
    (ev: any) => {
      if (!ev || !currentUserId) return;
      const callId = toId(ev.callId);
      const senderId = toId(ev.senderId);
      ...
      const parsedOffer = parseRealtimePayload(
        ev.offer || (ev.type === "offer" ? ev.signalData : null)
      );
      if (parsedOffer) incomingOfferRef.current = parsedOffer;
      const storedCandidates = Array.isArray(ev.candidates) ? ev.candidates : [];
      const parsedCandidate = parseRealtimePayload(
        ev.candidate ||
          (ev.type === "ice-candidate" || ev.type === "ice" ? ev.signalData : null)
      );
      if (storedCandidates.length > 0) iceCandidateQueueRef.current = storedCandidates;
      if (parsedCandidate) iceCandidateQueueRef.current.push(parsedCandidate);
      ...
      if (callSessionRef.current?.id === callId) return;
      const mode: CallMode = ev.callType === "audio" ? "audio" : "video";
      const caller = conversationsRef.current.find(
        (c) => getConversationPeerId(c) === senderId
      );
      const incoming: CallSession = {
        id: callId,
        mode,
    ...
    [currentUserId, getConversationPeerId, getAvatarUrl]
  );

  // ==================== DATA LOADING ====================
  const loadChatsAndFriends = useCallback(
    async (options: { force?: boolean; silent?: boolean } = {}) => {
      ...
      const cached = options.force
        ? null
        : readExpiringCache(chatListMemoryCache, cacheKey, CHAT_LIST_CACHE_TTL_MS);
      if (cached) {
        setConversations(cached);
        setIsLoadingChats(false);
        return;
      ...
      try {
        ...
        try {
          ...
          convs = raw.map((c: any) => ({
            ...
            name:
              c.type === "group"
                ? c.groupName || "Nhóm chat"
                : c.targetUserName || "Người dùng",
            ...
            isOnline: Boolean(c.targetIsOnline),
        ...
        friends.forEach((f: any) => {
          ...
          if (!existingIds.has(fId)) {
            final.push({
              ...
              avatar: f.avatar,
              lastMessage: "Sẵn sàng mở cuộc trò chuyện.",
              time: "",
              unread: 0,
  ...
  const handleAcceptRequest = useCallback(async () => {
    ...
    const selChat = conversationsRef.current.find(
      (c) => c.id === selectedChatIdRef.current
    ) as ConversationItem | undefined;
    if (!selChat) return;
    setIsAcceptingRequest(true);
    const convId = selChat.id.startsWith("new_") ? "" : selChat.id;
    try {
      if (!convId) throw new Error("Không thể chấp nhận hội thoại chưa có id.");
      const res = await api.request("PUT", `/api/chat/conversations/${convId}/accept`);
      const accepted = res?.data || {};
      setConversations((prev) =>
        prev.map((c) =>
          c.id === selChat.id
            ? { ...c, ...accepted, id: convId, status: "accepted", unread: c.unread }
            : c
        )
      );
      await loadChatsAndFriends({ force: true });
  ...
  const handleRejectRequest = useCallback(async () => {
    ...
    setConversations((prev) => prev.filter((c) => c.id !== selChat.id));
    setSelectedChatId(null);
    if (succeeded) toast.success("Đã từ chối tin nhắn");
    else toast.warning("Đã xóa khỏi giao diện. Backend cần deploy endpoint reject mới.");
  }, []);
  ...
  const startCall = useCallback(
    async (mode: CallMode) => {
      ...
      if (!selChat) {
        toast.error("Hãy chọn một cuộc trò chuyện trước.");
        return;
      }
      if (selChat.status === "pending") {
        toast.error("Không thể gọi khi hội thoại đang chờ phê duyệt.");
        return;
      }
      closeCall(false);
      const callId = `call_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
      const targetId = getConversationPeerId(selChat);
      const avatar = normalizeAvatarUrl(selChat.avatar, targetId || selChat.id);
      ...
      try {
        if (!navigator.mediaDevices?.getUserMedia)
          throw new Error("Trình duyệt không hỗ trợ media devices.");
        const stream = await getCallMediaStream(mode);
        localStreamRef.current = stream;
        if (mode === "video" && stream.getVideoTracks().length > 0)
          cameraStreamRef.current = stream.clone();
        if (localVideoRef.current) {
    ...
    [closeCall, getConversationPeerId, getCallMediaStream, createPeerConnection, sendCallSignal]
  );
  ...
  const acceptCall = useCallback(async () => {
    ...
    try {
      const stream = await getCallMediaStream(s.mode);
      localStreamRef.current = stream;
      if (s.mode === "video" && stream.getVideoTracks().length > 0)
        cameraStreamRef.current = stream.clone();
      if (localVideoRef.current) {
        localVideoRef.current.srcObject = stream;
        localVideoRef.current.play().catch(() => {});
      ...
      toast.error(`Lỗi thiết bị: ${err.message}`);
      closeCall(true, true);
    }
  }, [getCallMediaStream, createPeerConnection, sendCallSignal, closeCall]);

  const rejectCall = useCallback(() => closeCall(true, true), [closeCall]);

  const toggleMute = useCallback(() => {
    setCallSession((prev) => {
      if (!prev) return prev;
      const m = !prev.isMuted;
      localStreamRef.current?.getAudioTracks().forEach((t) => (t.enabled = !m));
      return { ...prev, isMuted: m };
    });
  }, []);
  ...
  const toggleScreenShare = useCallback(async () => {
    ...
    try {
      ...
      replaceLocalVideoTrack(screenTrack);
      setCallSession((prev) =>
        prev ? { ...prev, isScreenSharing: true, isCameraOff: false } : prev
      );
    } catch (error: any) {
      toast.error(error?.message || "Không thể chia sẻ màn hình.");
    }
  }, [stopScreenShare, replaceLocalVideoTrack]);
  ...
  const switchCamera = useCallback(async () => {
    ...
    try {
      ...
      replaceLocalVideoTrack(track);
      setCallSession((prev) =>
        prev ? { ...prev, cameraFacing: nextFacing, isCameraOff: false } : prev
      );
    } catch {
      toast.error("Thiết bị không có camera trước/sau để đổi.");
    }
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        const subscribeUserQueue = (queue: string, handler: (frame: any) => void) => {
          [`/user/queue/${queue}`, `/user/${currentUserId}/queue/${queue}`].forEach((dest) => {
        ...
        subscribeUserQueue("messages", (frame) => {
          ...
          const messageKey =
            toId(msg.id) ||
            `${toId(msg.conversationId)}:${toId(msg.senderId)}:${toId(msg.createdAt)}:${toId(msg.content)}`;
          if (messageKey && realtimeMessageIdsRef.current.has(messageKey)) return;
          if (messageKey) {
            realtimeMessageIdsRef.current.add(messageKey);
            if (realtimeMessageIdsRef.current.size > 300) {
              realtimeMessageIdsRef.current = new Set(
                Array.from(realtimeMessageIdsRef.current).slice(-150)
              );
            }
          }
          ...
          setConversations((prev) => {
            ...
            const idx = updated.findIndex(
              (c) =>
                (convId && toId(c.id) === convId) ||
                (peerId && getConversationPeerId(c) === peerId)
            );
            if (idx > -1) {
              const conv = { ...updated[idx] };
              conv.lastMessage = msg.content;
              conv.time = new Date(msg.createdAt).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              });
              if (
                senderId !== currentUserId &&
                !doesEventMatchSelectedChat(convId, senderId, receiverId)
              )
              ...
              if (toId(conv.id).startsWith("new_") && convId) {
                ...
                if (
                  selectedChatIdRef.current === `new_${prevPeerId}` ||
                  selectedChatIdRef.current === `new_${peerId}`
                )
          ...
          if (doesEventMatchSelectedChat(convId, senderId, receiverId)) {
            setMessages((prev) => {
              ...
              const clean = prev.filter(
                (m) => !(m.id.startsWith("temp_") && m.text === msg.content)
              );
              return [
                ...clean,
                {
            ...
            prev.map((item) => {
              ...
              return {
                ...
                text: msg.isDeleted
                  ...
                  : msg.content || msg.text || item.text,
                isDeleted: Boolean(msg.isDeleted),
                isEdited: Boolean(msg.editedAt),
                time: msg.createdAt
        ...
        subscribeUserQueue("typing", (frame) => {
          const ev = safe(frame.body);
          if (!ev) return;
          const isTyping = ev.isTyping === true || ev.typing === true;
          const convKey = toId(ev.conversationId);
          const peerId = getEventPeerId(ev.senderId, ev.receiverId);
          setTypingUsers((prev) => {
            const next = { ...prev };
        ...
        subscribeUserQueue("call", (frame) => {
          ...
          if (type === "start") {
            const caller = conversationsRef.current.find(
              (c) => getConversationPeerId(c) === toId(senderId)
            );
            const incoming: CallSession = {
              id: callId,
              mode: callType,
              ...
              peerName: caller?.name || "Người gọi",
            ...
            peerConnectionRef.current
              .setRemoteDescription(new RTCSessionDescription(parsed))
              .then(() => {
                iceCandidateQueueRef.current.forEach((c) =>
              ...
              .catch(console.error);
            const next = callSessionRef.current
            ...
            if (peerConnectionRef.current?.remoteDescription)
              peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(parsed)).catch(() => {});
            else iceCandidateQueueRef.current.push(parsed);
          } else if (type === "reject" || type === "end") {
        ...
        subscribeUserQueue("reactions", (frame) => {
          const reaction = safe(frame.body);
          if (!reaction) return;
          setMessages((prev) =>
            prev.map((msg) => {
              if (msg.id !== reaction.messageId) return msg;
        ...
        subscribeUserQueue("online-status", (frame) => {
          const status = safe(frame.body);
          if (!status) return;
          const userId = toId(status.userId);
          if (!userId) return;
          setConversations((prev) =>
            prev.map((c) =>
              getConversationPeerId(c) === userId
                ? { ...c, isOnline: Boolean(status.isOnline) }
                : c
            )
          );
          setSearchResults((prev) =>
            prev.map((u) =>
              toId(u.id) === userId ? { ...u, isOnline: Boolean(status.isOnline) } : u
            )
          );
        });
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(INCOMING_CALL_EVENT, onIncomingCall);
  }, [currentUserId, applyIncomingCallEvent]);

  useEffect(() => {
    if (!currentUserId) return;
    loadChatsAndFriends();
    const refreshTimer = window.setInterval(
      () => loadChatsAndFriends({ silent: true }),
      60000
    );
    return () => window.clearInterval(refreshTimer);
  }, [currentUserId, loadChatsAndFriends]);
  ...
  useEffect(() => {
    ...
    const existing = conversations.find(
      (chat) => getConversationPeerId(chat) === userId || chat.id === userId
    );
    if (existing) {
      setSelectedChatId(existing.id);
      return;
    ...
    const openNewConversation = async () => {
      try {
        ...
        setSearchResults((prev) =>
          prev.some((u) => toId(u.id) === toId(item.id)) ? prev : [item, ...prev]
        );
        setSelectedChatId(`new_${toId(item.id)}`);
      } catch {
        toast.error("Không thể mở hội thoại với người dùng này.");
  ...
  }, [location.search, isLoadingChats, conversations, getConversationPeerId]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setIsSearching(true);
      try {
        const r = await api.searchUsersToChat(searchQuery);
        setSearchResults(r || []);
      } catch {
        // ignore
      } finally {
  ...
  useEffect(() => {
    ...
    if (!currentUserId) return;
    const cacheKey = `messages:${currentUserId}:${selectedChatId}`;
    const selChat = conversationsRef.current.find((c) => c.id === selectedChatId);
    const hasUnread = Number(selChat?.unread || 0) > 0;
    ...
    const fetchMessages = async () => {
      ...
      try {
        const res = await api.request("GET", `/api/chat/messages/${selectedChatId}`);
        const list = Array.isArray(res) ? res : res?.data || [];
        const nextMessages: MessageItem[] = list.map((m: any) => ({
          id: toId(m.id) || Math.random().toString(),
          senderId: toId(m.senderId) === currentUserId ? "me" : toId(m.senderId),
          senderName: m.senderName,
          senderAvatar: m.senderAvatar,
          text: m.isDeleted
            ? "Tin nhắn đã được thu hồi"
            : m.content || m.text || "Tin nhắn không hợp lệ.",
  ...
  // Visual viewport height for mobile
  useEffect(() => {
    const root = document.documentElement;
    let raf = 0;
    const updateVisualHeight = () => {
  ...
  const handleSendMessage = useCallback(
    async (event: FormEvent) => {
      ...
      if (!messageInput.trim() || !selChat) return;
      ...
      setConversations((prev) => {
        const updated = [...prev];
        const idx = updated.findIndex((c) => c.id === selChat.id);
        if (idx > -1) {
          const conv = { ...updated[idx], lastMessage: `Bạn: ${content}`, time: opt.time };
          updated.splice(idx, 1);
          updated.unshift(conv);
        }
        return dedupConversations(updated);
      ...
      const payload = {
        conversationId: selChat.id.startsWith("new_") ? "" : selChat.id,
        receiverId: getConversationPeerId(selChat),
        content,
        messageType: "text",
        replyToMessageId: currentReply?.id,
      };
    ...
    [messageInput, replyingTo, getConversationPeerId]
  );

  const handleEditMessage = useCallback(async (message: MessageItem) => {
    if (message.senderId !== "me" || message.isDeleted || message.id.startsWith("temp_")) return;
    const content = window.prompt("Sửa tin nhắn", message.text);
    if (content === null) return;
    const nextContent = content.trim();
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((item) =>
          item.id === message.id
            ? { ...item, text: updated.content || nextContent, isEdited: true }
            : item
        )
      );
    } catch (error: any) {
  ...
  const handleRecallMessage = useCallback(async (message: MessageItem) => {
    if (message.senderId !== "me" || message.isDeleted || message.id.startsWith("temp_")) return;
    if (!window.confirm("Thu hồi tin nhắn này?")) return;
    try {
      await api.request("DELETE", `/api/chat/messages/${encodeURIComponent(message.id)}`);
      setMessages((prev) =>
        prev.map((item) =>
          item.id === message.id
            ? { ...item, text: "Tin nhắn đã được thu hồi", isDeleted: true }
            : item
        )
      );
    } catch (error: any) {
  ...
  const handleChatBackgroundChange = useCallback(
  ...
  const handleInputChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (stompClientRef.current?.connected && selChat) {
        const payload = {
          conversationId: selChat.id.startsWith("new_") ? "" : selChat.id,
          receiverId: getConversationPeerId(selChat),
          isTyping: true,
  ...
  const openSelectedProfile = useCallback(() => {
    ...
    const peerId = selChat ? getConversationPeerId(selChat) : "";
    if (!peerId) return;
    navigate(`/nguoi-dung/${peerId}`);
  }, [getConversationPeerId, navigate]);
  ...
  const displayList: ConversationItem[] = isSearchMode
    ? searchResults.map((u) => ({
        ...
        name: u.name,
        avatar: u.avatar,
        lastMessage: "Nhắn để mở cuộc trò chuyện ngay.",
        time: "",
        unread: 0,
        isOnline: Boolean(u.isOnline),
        status: "accepted",
  ...
  const isSelectedTyping = selectedChat
    ? Boolean(
        typingUsers[selectedChat.id] ||
          (selectedPeerId && typingUsers[`new_${selectedPeerId}`])
      )
    : false;
  const selectedStatus = isSelectedTyping
    ? "Đang soạn tin nhắn..."
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 8px" }}>
          {isLoadingChats || isSearching ? (
            <div
              style={{ display: "flex", justifyContent: "center", padding: 32, color: ORANGE }}
            >
              <Loader2 size={24} style={{ animation: "spin 1s linear infinite" }} />
            </div>
          ) : displayList.length === 0 ? (
            <div
              style={{ textAlign: "center", padding: "48px 16px", color: "#b0b0b0" }}
            >
              <Inbox size={40} style={{ margin: "0 auto 12px", opacity: 0.4 }} />
              <div style={{ fontSize: 14 }}>
                {isSearchMode
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    border: "1px solid #f0f0f0",
                    background: showInfo ? ORANGE_LIGHT : "#fff",
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
            ...
            <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
              ...
              <div
                style={{
                  ...
                  flexDirection: "column",
                  minWidth: 0,
                  background: selectedChatBackgroundValue,
                }}
              >
                <div
                ...
                >
                  {isLoadingMessages ? (
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 13, color: "#b0b0b0", maxWidth: 280 }}
                      >
                        Hãy nhắn tin trước để bắt đầu!
                      </div>
                    </div>
                  ) : (
                    <div
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              style={{ maxWidth: "68%", position: "relative" }}
                            >
                              {!isMe && isGroupChat && msg.senderName && (
                                <div
                                  style={{
                              ...
                              <div
                                style={{
                                  background: msg.isDeleted
                                    ? "#f8fafc"
                                    : isMe
                                    ? `linear-gradient(135deg, ${ORANGE}, ${ORANGE_MID})`
                                    : "#fff",
                                  color: msg.isDeleted
                                    ? "#94a3b8"
                                    : isMe
                                ...
                                onMouseEnter={() =>
                                  !msg.isDeleted && setShowReactionPicker(msg.id)
                                }
                                onContextMenu={(event) => {
                                  event.preventDefault();
                                  if (!msg.isDeleted)
                              ...
                              >
                                {repliedMessage && (
                                  <div
                                    style={{
                                      borderLeft: `3px solid ${isMe ? "#fff" : ORANGE}`,
                                      background: isMe
                                ...
                                {msg.attachmentUrl && !msg.isDeleted &&
                                  (String(msg.messageType).toLowerCase() === "image" ? (
                                    <img
                                      src={normalizeAvatarUrl(msg.attachmentUrl, msg.id)}
                                    ...
                                    <a
                                      href={normalizeAvatarUrl(msg.attachmentUrl, msg.id)}
                                      target="_blank"
                                      rel="noreferrer"
                                      style={{
                                        display: "flex",
                              ...
                              >
                                {msg.time}
                                {msg.isEdited && !msg.isDeleted ? " · đã sửa" : ""}
                              </div>
                                        ...
                                        count > 0 && (
                                          <button
                                            ...
                                            style={{
                                              ...
                                              borderRadius: 12,
                                              border: `1px solid ${
                                                msg.userReactions?.[emoji] ? ORANGE : "#f0f0f0"
                                              }`,
                                              background: msg.userReactions?.[emoji]
                                                ? ORANGE_LIGHT
                                                : "#fff",
                                              cursor: "pointer",
                                              fontSize: 12,
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: ORANGE,
                            color: "#fff",
                            cursor: isAcceptingRequest ? "not-allowed" : "pointer",
                            fontSize: 14,
                            fontWeight: 600,
                            display: "flex",
                            alignItems: "center",
                            gap: 6,
                            opacity: isAcceptingRequest ? 0.7 : 1,
                            boxShadow: `0 2px 8px ${ORANGE}50`,
                          }}
                        >
                          {isAcceptingRequest ? (
                        ...
                        <div style={{ minWidth: 0, fontSize: 12 }}>
                          <div style={{ fontWeight: 800, color: ORANGE }}>
                            ...
                            {replyingTo.senderId === "me"
                              ? "Bạn"
                              : replyingTo.senderName || selectedChat.name}
                          </div>
                          <div
                            style={{
                              overflow: "hidden",
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 600,
                            background:
                              infoTab === tab.value ? "#fff" : "transparent",
                            color:
                              infoTab === tab.value ? ORANGE : "#9ca3af",
                            boxShadow:
                              infoTab === tab.value
                  ...
                  <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
                    {infoTab === "info" ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                        <div
                          style={{
                            background: `linear-gradient(135deg, ${ORANGE_LIGHT}, #fff8f5)`,
                            borderRadius: 18,
                            padding: 18,
                            textAlign: "center",
                          }}
                        ...
                        >
                          {[
                            { label: "Tin nhắn", value: messages.length },
                            {
                              label: "Trạng thái",
                              value:
                                selectedChat.status === "pending" ? "Chờ" : "Kết nối",
                            },
                          ].map((stat) => (
                            <div
                              key={stat.label}
                              ...
                              return (
                                ...
                                >
                                  <img
                                    src={
                                      isMe
                                        ? getAvatarUrl(currentUser?.avatar, currentUserId)
                                        : getAvatarUrl(
                                            msg.senderAvatar || selectedChat.avatar,
                                            msg.senderId
                                          )
                                    }
                                    alt=""
            ...
            <div style={{ padding: 26 }}>
              ...
              >
                ...
                >
                  {callSession.mode === "video" ? (
                    <Camera size={13} color={ORANGE_MID} />
                  ) : (
                    <Phone size={13} color={ORANGE_MID} />
                  )}
                  <span
                    style={{
                      fontSize: 11,
              ...
              {callSession.status === "incoming" ? (
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 10 }}>
                  {[
                    {
                      icon: callSession.isMuted ? MicOff : Mic,
                      onClick: toggleMute,
                      active: callSession.isMuted,
                    },
                    ...(callSession.mode === "video"
                      ? [
                          {
                            icon: callSession.isCameraOff ? VideoOff : Video,
                            onClick: toggleCamera,
                            active: callSession.isCameraOff,
                          },
                        ]
                      : []),
                    ...(callSession.mode === "video"
                      ? [
                          {
                            icon: callSession.isScreenSharing
                              ? ScreenShareOff
                              : ScreenShare,
                            onClick: toggleScreenShare,
                            active: callSession.isScreenSharing,
                          },
                        ]
                      : []),
                    ...(callSession.mode === "video"
                      ? [{ icon: SwitchCamera, onClick: switchCamera, active: false }]
                      : []),
                    {
                      icon: Volume2,
                      onClick: toggleSpeaker,
                      active: !callSession.isSpeakerOn,
                    },
                  ].map((btn, i) => (
                    <button
                      key={i}
              ...
              >
                {dedupConversations([
                  ...conversations.filter(
                    (c) => c.type !== "group" && getConversationPeerId(c)
                  ),
                  ...searchResults.map((u) => ({
                    id: `new_${toId(u.id)}`,
                    targetUserId: toId(u.id),
                  ...
                  return (
                    <label
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        cursor: "pointer",
                        background: checked ? ORANGE_LIGHT : "#fff",
                      }}
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <img
                        src={getAvatarUrl(item.avatar, peerId)}
                        alt=""
                        style={{
                          width: 36,
                          height: 36,
      ...
      <style>{`
        ...
        .message-bubble-group:hover .message-action-row,
        .message-action-row[data-active="true"] {
          opacity: 1;
          pointer-events: auto;
        }
