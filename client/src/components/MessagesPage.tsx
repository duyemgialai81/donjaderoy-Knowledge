...
// // Cấu hình máy chủ STUN miễn phí của Google cho WebRTC
// const iceServers = {
...
//   // 1. KHAI BÁO CÁC HÀM HELPER LÊN TRƯỚC
//   const dedupConversations = (list: ConversationItem[]) => {
//     const map = new Map<string, ConversationItem>();
//     list.forEach((conversation) => {
//       const key =
//         conversation.targetUserId !== undefined && conversation.targetUserId !== null
//           ? String(conversation.targetUserId)
...
//     if (normalizedSenderId && normalizedSenderId === currentUserId) {
//       return normalizedReceiverId;
//     }

//     if (normalizedReceiverId && normalizedReceiverId === currentUserId) {
...
//     if (normalizedConversationId && normalizedConversationId === selectedId) {
//       return true;
//     }

//     const selectedConversation = conversationsRef.current.find((conversation) => toId(conversation.id) === selectedId);
...
//   // 2. KHAI BÁO BIẾN DÙNG TỚI CÁC HÀM TRÊN
...
//         isOnline: true,
//       })),
//     ].find((conversation) => conversation.id === selectedChatId);
...
//         convs = rawConvs.map((conversation: any) => ({
//           id: toId(conversation.id),
//           targetUserId: toId(conversation.targetUserId) || undefined,
//           name: conversation.targetUserName || "Nguoi dung",
//           avatar: conversation.targetUserAvatar,
//           lastMessage: conversation.lastMessage || "Bat dau cuoc tro chuyen moi.",
...
//       let friends: any[] = [];
//       try {
//         friends = (await api.getMutualFollowersForChat()) || [];
//       } catch (error) {
//         console.error("Loi tai danh sach ban be:", error);
//       }
//       const finalConversations = [...convs];
//       const existingTargetIds = new Set(convs.map((conversation) => String(conversation.targetUserId || conversation.id)));
...
//   const sendCallSignal = (type: string, payload: any = null, targetId: string, callId: string, callMode: CallMode) => {
//     if (stompClientRef.current?.connected) {
//       stompClientRef.current.publish({
//         destination: "/app/chat.call",
//         body: JSON.stringify({
//           callId,
//           conversationId: selectedChatIdRef.current?.startsWith("new_") ? "" : (selectedChatIdRef.current || ""),
//           receiverId: targetId,
//           type,
//           callType: callMode,
...
//       if (event.candidate) {
//         sendCallSignal("ice", event.candidate, targetId, callId, callMode);
//       }
//     };
...
//   const closeCall = (isLocal = true) => {
//     const currentSession = callSessionRef.current;
    
//     if (currentSession && isLocal && currentSession.status !== "incoming") {
//       sendCallSignal("end", null, currentSession.peerId, currentSession.id, currentSession.mode);
//     }
...
//     if (!selectedChat) {
//       toast.error("Hay chon mot cuoc tro chuyen truoc.");
//       return;
//     }

//     if (selectedChat.status === "pending") {
//       toast.error("Khong the goi khi hoi thoai van dang cho phe duyet.");
//       return;
//     }
...
//         throw new Error("Trinh duyet hien tai khong ho tro media devices.");
//       }
...
//       sendCallSignal("offer", offer, targetId, callId, mode);
      
//     } catch (error) {
//       toast.error("Khong the truy cap camera va micro.");
//       setCallSession((prev) => prev ? { ...prev, hasMediaPermission: false, error: "Loi truy cap phuong tien" } : prev);
//       closeCall(true);
//     }
//   };
...
//       const stream = await navigator.mediaDevices.getUserMedia({ 
//         audio: true, 
//         video: currentSession.mode === "video" 
//       });
//       localStreamRef.current = stream;
...
//         sendCallSignal("answer", answer, currentSession.peerId, currentSession.id, currentSession.mode);
//         setCallSession(prev => prev ? { ...prev, status: "active", startedAt: Date.now() } : prev);
//       }
//     } catch (err) {
//       toast.error("Khong the lay thiet bi phuong tien. Tu choi cuoc goi.");
//       rejectCall();
//     }
//   };
...
//     const currentSession = callSessionRef.current;
//     if (currentSession) {
//       sendCallSignal("reject", null, currentSession.peerId, currentSession.id, currentSession.mode);
//     }
//     closeCall(false);
//   };
...
//       const nextMuted = !previous.isMuted;
//       localStreamRef.current?.getAudioTracks().forEach((track) => {
//         track.enabled = !nextMuted;
//       });
//       return { ...previous, isMuted: nextMuted };
//     });
//   };

//   const toggleCamera = () => {
//     setCallSession((previous) => {
//       if (!previous || previous.mode !== "video") return previous;
//       const nextCameraOff = !previous.isCameraOff;
//       localStreamRef.current?.getVideoTracks().forEach((track) => {
//         track.enabled = !nextCameraOff;
//       });
//       return { ...previous, isCameraOff: nextCameraOff };
//     });
//   };

//   const toggleSpeaker = () => {
//     setCallSession((previous) =>
//       previous ? { ...previous, isSpeakerOn: !previous.isSpeakerOn } : previous,
//     );
//   };
...
//   useEffect(() => {
//     if (!callSession || callSession.status !== "active" || !callSession.startedAt) {
//       return;
//     }
...
//           ? {
//               ...previous,
//               elapsedSeconds: Math.floor((Date.now() - previous.startedAt) / 1000),
//             }
//           : previous,
//       );
//     }, 1000);
...
//           const { type, callId, callType, senderId, payload } = event;

//           if (type === "start") {
//             const caller = conversationsRef.current.find((conversation) => getConversationPeerId(conversation) === toId(senderId));
//             setCallSession({
//               id: callId,
//               mode: callType,
//               status: "incoming",
...
//               isSpeakerOn: true,
//               peerId: toId(senderId),
//               peerName: caller?.name || "Nguoi goi",
//               peerAvatar: caller?.avatar || getAvatarUrl("", senderId),
//               hasMediaPermission: true,
//               error: null,
//             });
//           } else if (type === "offer") {
//             if (payload && payload !== "undefined") {
//                incomingOfferRef.current = JSON.parse(payload);
//             }
//           } else if (type === "answer") {
//             if (payload && payload !== "undefined") {
//                 const answerDesc = new RTCSessionDescription(JSON.parse(payload));
//                 peerConnectionRef.current?.setRemoteDescription(answerDesc).catch(console.error);
//                 setCallSession(prev => prev ? { ...prev, status: "active", startedAt: Date.now() } : prev);
//             }
//           } else if (type === "ice") {
//             if (payload && payload !== "undefined") {
...
//             toast.info(type === "reject" ? "Cuoc goi bi tu choi" : "Cuoc goi da ket thuc");
//             closeCall(false);
//           }
//         });
...
//           const event = JSON.parse(frame.body);
//           const isUserTyping = event.isTyping === true || event.typing === true;
//           const conversationKey = toId(event.conversationId);
//           const peerId = getEventPeerId(event.senderId, event.receiverId);

//           setTypingUsers((previous) => {
//             const nextTypingUsers = { ...previous };

//             if (conversationKey) {
//               nextTypingUsers[conversationKey] = isUserTyping;
//             }

//             if (peerId) {
//               nextTypingUsers[`new_${peerId}`] = isUserTyping;
//             }
...
//     const currentChat = conversations.find((conversation) => conversation.id === selectedChatId);
//     if (!currentChat) return;

//     const expectedStatus = activeTab === "inbox" ? "accepted" : "pending";
//     if (currentChat.status !== expectedStatus) {
...
//       setIsSearching(true);
//       try {
//         const results = await api.searchUsersToChat(searchQuery);
...
//         const formattedMessages = dataList.map((message: any) => ({
//           id: toId(message.id) || Math.random().toString(),
//           senderId: toId(message.senderId) === currentUserId ? "me" : toId(message.senderId),
//           text: message.content || message.text || "Tin nhan hien thi khong hop le.",
...
//       const conversationIndex = updatedConversations.findIndex((conversation) => conversation.id === selectedChat.id);

//       if (conversationIndex > -1) {
//         const updatedConversation = {
//           ...updatedConversations[conversationIndex],
//           lastMessage: `Ban: ${content}`,
//           time: optimisticMessage.time,
//         };
//         updatedConversations.splice(conversationIndex, 1);
//         updatedConversations.unshift(updatedConversation);
//       } else if (selectedChatId?.startsWith("new_")) {
...
//         });
//       }
//       return dedupConversations(updatedConversations);
//     });
...
//           stompClientRef.current.publish({
//             destination: "/app/chat.typing",
//             body: JSON.stringify({ ...typingPayload, isTyping: false, typing: false }),
//           });
//         }
//       }, 2000);
//     }
...
//   const inboxConversations = conversations.filter((conversation) => conversation.status === "accepted");
//   const pendingConversations = conversations.filter((conversation) => conversation.status === "pending");
//   const totalUnread = inboxConversations.reduce((sum, conversation) => sum + (conversation.unread || 0), 0);
//   const onlineCount = inboxConversations.filter((conversation) => conversation.isOnline).length;
...
//         name: user.name,
//         avatar: user.avatar,
//         lastMessage: "Nhan de mo cuoc tro chuyen ngay.",
//         time: "",
//         unread: 0,
//         isOnline: true,
//         status: "accepted",
...
//         activeTab === "inbox" ? conversation.status === "accepted" : conversation.status === "pending",
//       );

//   const selectedPeerId = selectedChat ? getConversationPeerId(selectedChat) : "";
//   const isSelectedTyping = selectedChat
//     ? Boolean(typingUsers[selectedChat.id] || (selectedPeerId && typingUsers[`new_${selectedPeerId}`]))
//     : false;
//   const selectedResponseState = isSelectedTyping
//     ? "Dang soan tin nhan"
...
//           icon: BellRing,
//           title: "Do uu tien",
//           value: selectedChat.unread ? "Can xu ly" : "On dinh",
//         },
//         {
//           icon: Clock3,
//           title: "Lan trao doi",
//           value: selectedChat.time || "Moi bat dau",
//         },
//         {
//           icon: Star,
//           title: "Trang thai",
//           value: selectedChat.status === "pending" ? "Cho phe duyet" : "San sang ket noi",
//         },
//       ]
//     : [];
...
//     {
//       label: "Tin cho",
//       value: pendingConversations.length,
...
//         <div className="flex h-full min-h-0 w-full overflow-hidden rounded-[32px] border border-orange-100 bg-white shadow-[0_20px_56px_rgba(249,115,22,0.06)]">
//           <aside
//             className={`${selectedChat ? "hidden xl:flex" : "flex"} messenger-sidebar w-full shrink-0 flex-col border-r border-orange-100 bg-white xl:w-[336px]`}
//           >
...
//                     Hop thu trung tam cho trao doi, goi nhanh va quan ly hoi thoai.
//                   </p>
//                 </div>
//                 <Button
//                   variant="ghost"
//                   size="icon"
//                   className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-500 hover:bg-orange-50 hover:text-orange-600"
//                 >
...
//                 <Input
//                   placeholder="Tim nguoi dung, ten doi thoai..."
//                   className="h-12 rounded-2xl border-orange-100 bg-white pl-11 shadow-sm focus-visible:ring-orange-500"
//                   value={searchQuery}
...
//                     <TabsTrigger
//                       value="inbox"
//                       className="rounded-xl text-sm font-medium data-[state=active]:bg-white data-[state=active]:text-orange-600 data-[state=active]:shadow-sm"
//                     >
//                       Hop thu
//                     </TabsTrigger>
//                     <TabsTrigger
...
//                           setShowInfo(false);
//                         }}
//                         className={`w-full rounded-[26px] border p-3 text-left transition-all ${
//                           isActive
//                             ? "border-orange-200 bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_100%)] shadow-[0_14px_38px_rgba(249,115,22,0.12)]"
//                             : "border-transparent bg-white hover:border-orange-100 hover:bg-orange-50/40"
//                         }`}
//                       >
...
//                               src={avatarUrl}
//                               alt={chat.name}
//                               className="h-14 w-14 rounded-[20px] border border-orange-100 object-cover shadow-sm"
//                             />
...
//                                   </span>
//                                   {chat.time ? <span className="text-[11px] text-slate-400">{chat.time}</span> : null}
//                                 </div>
//                               </div>

//                               {chat.unread ? (
//                                 <span className="inline-flex h-6 min-w-[24px] items-center justify-center rounded-full bg-slate-950 px-2 text-[11px] font-semibold text-white">
//                                   {chat.unread}
//                                 </span>
//                               ) : null}
//                             </div>

//                             <p className="messenger-preview mt-3 text-sm leading-6 text-slate-500">
//                               {typingUsers[chat.id]
//                                 ? "Dang nhap noi dung..."
//                                 : chat.lastMessage || "Nhan de mo cuoc tro chuyen ngay."}
//                             </p>
//                           </div>
//                         </div>
//                       </button>
...
//                     {isSearchMode ? "Khong tim thay ket qua phu hop" : "Chua co hoi thoai nao"}
//                   </div>
//                   <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
//                     {isSearchMode
//                       ? "Thu mo rong tu khoa, ten day du hoac bat dau hoi thoai tu danh sach ban be."
...
//           <section className={`${selectedChat ? "flex" : "hidden xl:flex"} messenger-thread min-w-0 flex-1 flex-col bg-white`}>
//             {selectedChat ? (
//               <>
//                 <div className="border-b border-orange-100 bg-white/95 px-4 py-4 backdrop-blur sm:px-6">
...
//                         variant="ghost"
//                         size="icon"
//                         className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600 xl:hidden"
...
//                           className="h-12 w-12 rounded-[20px] border border-orange-100 object-cover shadow-sm"
//                         />
//                         {selectedChat.isOnline ? (
//                           <span className="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-[3px] border-white bg-emerald-500" />
//                         ) : null}
...
//                         variant="ghost"
//                         size="icon"
//                         className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600"
...
//                         variant="ghost"
//                         size="icon"
//                         className={`h-10 w-10 rounded-full border border-orange-100 bg-white transition-colors ${
...
//                       <div
//                         key={item.title}
//                         className="inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50/70 px-3 py-2 text-xs font-medium text-slate-600"
//                       >
...
//                               src={selectedChatAvatar}
//                               alt={selectedChat.name}
//                               className="relative h-24 w-24 rounded-[28px] border-4 border-white object-cover shadow-xl"
//                             />
//                           </div>
//                           <div className="rounded-full border border-orange-100 bg-orange-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-orange-700">
//                             Khoi dong cuoc tro chuyen
//                           </div>
//                           <h3 className="mt-5 text-2xl font-semibold text-slate-950">{selectedChat.name}</h3>
//                           <p className="mt-3 max-w-xl text-sm leading-7 text-slate-500">
//                             Day la khung chat da san sang cho goi video, goi thoai va trao doi tai lieu. Ban co the bat dau bang mot thong diep ngan, ro rang.
//                           </p>
...
//                                 className="messenger-floating-card rounded-3xl p-4 text-left transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md"
//                               >
//                                 <div className="mb-3 inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-orange-100 text-orange-600">
...
//                             <div className="rounded-full border border-orange-100 bg-white px-4 py-2 text-xs font-medium text-slate-500 shadow-sm">
...
//                                         src={selectedChatAvatar}
//                                         alt=""
//                                         className="h-9 w-9 rounded-2xl border border-orange-100 object-cover shadow-sm"
//                                       />
//                                     ) : (
...
//                                   <span className="shrink-0 pb-1 text-[11px] font-medium text-slate-400">
//                                     {message.time}
//                                   </span>
//                                   <div
//                                     className={`rounded-[22px] px-4 py-1.5 text-[14px] leading-6 ${
//                                       isMe
//                                         ? "messenger-bubble-outgoing rounded-br-lg text-white"
//                                         : "messenger-bubble-incoming rounded-bl-lg text-slate-800"
//                                     }`}
//                                   >
//                                     {message.text}
//                                   </div>
...
//                             <Button variant="outline" className="h-11 flex-1 rounded-2xl border-orange-100 bg-white hover:bg-orange-50">
//                               Tu choi
//                             </Button>
//                             <Button className="h-11 flex-1 rounded-2xl bg-orange-500 text-white hover:bg-orange-600">
...
//                                 className="rounded-full border border-orange-100 bg-orange-50/70 px-3 py-2 text-xs font-medium text-slate-600 transition hover:border-orange-200 hover:bg-orange-100 hover:text-orange-700"
//                               >
//                                 {reply}
//                               </button>
//                             ))}
...
//                               variant="ghost"
//                               size="icon"
//                               className="h-11 w-11 shrink-0 rounded-2xl border border-orange-100 bg-white text-slate-500 hover:bg-orange-50 hover:text-orange-600"
//                             >
...
//                               <input
//                                 type="text"
//                                 placeholder="Nhap tin nhan, chia se tai lieu, hoac bat dau cuoc goi..."
//                                 value={messageInput}
//                                 onChange={handleInputChange}
//                                 className="min-h-[28px] flex-1 bg-transparent py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400"
//                               />
//                               <button
//                                 type="button"
//                                 className="mb-1 ml-3 rounded-full p-1 text-slate-400 transition hover:bg-orange-50 hover:text-orange-600"
//                               >
...
//                                 type="submit"
//                                 size="icon"
//                                 className="h-11 w-11 shrink-0 rounded-2xl bg-orange-500 text-white shadow-lg shadow-orange-200 hover:bg-orange-600"
//                               >
...
//                                 variant="ghost"
//                                 size="icon"
//                                 className="h-11 w-11 shrink-0 rounded-2xl border border-orange-100 bg-white text-orange-600 hover:bg-orange-50"
//                               >
...
//                         className="absolute inset-0 z-10 bg-slate-950/10 2xl:hidden"
...
//                               src={selectedChatAvatar}
//                               alt={selectedChat.name}
//                               className="mx-auto h-20 w-20 rounded-[24px] border-4 border-white object-cover shadow-lg"
//                             />
//                             <h3 className="mt-4 text-lg font-semibold text-slate-950">{selectedChat.name}</h3>
//                             <p className="mt-1 text-sm text-slate-500">{selectedResponseState}</p>
//                             <div className="mt-4 flex justify-center gap-2">
//                               <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-orange-700">
...
//                               <Button
//                                 variant="ghost"
//                                 className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50"
//                               >
...
//                                 onClick={() => startCall("video")}
//                               >
//                                 <Video className="mr-3 h-4 w-4 text-slate-400" />
//                                 Bat dau video call
//                               </Button>
//                               <Button
//                                 variant="ghost"
//                                 className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50"
//                               >
//                                 <Archive className="mr-3 h-4 w-4 text-slate-400" />
//                                 Luu tru hoi thoai
//                               </Button>
//                             </div>
//                           </div>
...
//                                 Neu phat hien spam hoac quang cao, ban co the chan nguoi dung nay de ngan tin nhan trong tuong lai.
//                               </p>
//                               <Button
//                                 variant="ghost"
//                                 className="mt-3 h-11 w-full justify-start rounded-2xl bg-white text-red-600 hover:bg-red-100 hover:text-red-700"
//                               >
...
//                   <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-[28px] bg-white shadow-xl shadow-orange-100">
//                     <MessageCircle className="h-10 w-10 text-orange-500" />
//                   </div>
//                   <div className="mx-auto max-w-sm rounded-full border border-orange-100 bg-orange-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-orange-700">
//                     Messaging hub
//                   </div>
//                   <h2 className="mt-6 text-3xl font-semibold text-slate-950">Tap trung tat ca trao doi vao mot noi</h2>
//                   <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">
//                     Chon mot hoi thoai ben trai de nhan tin, goi audio, video call va quan ly thong tin lien quan trong mot khung UI dong bo.
//                   </p>

//                   <div className="mt-8 grid gap-4 text-left sm:grid-cols-3">
//                     <div className="rounded-[28px] border border-orange-100 bg-white/95 p-5 shadow-sm">
//                       <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-orange-100 text-orange-600">
...
//                     </div>
//                     <div className="rounded-[28px] border border-orange-100 bg-white/95 p-5 shadow-sm">
//                       <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-100 text-orange-600">
...
//                     variant="ghost"
//                     size="icon"
//                     className="h-10 w-10 rounded-full bg-white/10 text-white hover:bg-white/15"
...
//                         <Button size="icon" className="h-16 w-16 rounded-full bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30" onClick={rejectCall}>
//                           <PhoneOff className="h-6 w-6 text-white" />
//                         </Button>
//                         <Button size="icon" className="h-16 w-16 rounded-full bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-500/30" onClick={acceptCall}>
//                           {callSession.mode === "video" ? <Video className="h-6 w-6 text-white" /> : <Phone className="h-6 w-6 text-white" />}
//                         </Button>
//                       </div>
//                     </div>
...
//                             <div className="mb-4 flex h-24 w-24 items-center justify-center rounded-[28px] bg-white/10">
...
//                           className="relative h-32 w-32 rounded-[32px] border-4 border-white/15 object-cover shadow-2xl"
//                         />
//                       </div>
//                       <div className="text-3xl font-semibold text-white">{callSession.peerName}</div>
...
//                       </div>
//                       {/* Thẻ audio ẩn để phát tiếng của người bên kia */}
...
//                         callSession.isMuted ? "bg-red-500 text-white hover:bg-red-600" : "bg-white/10 text-white hover:bg-white/15"
//                       }`}
//                       onClick={toggleMute}
//                     >
//                       {callSession.isMuted ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
//                     </Button>
//                     {callSession.mode === "video" ? (
//                       <Button
//                         type="button"
...
//                         size="icon"
//                         className={`h-14 w-14 rounded-full border border-white/10 ${
//                           callSession.isCameraOff ? "bg-red-500 text-white hover:bg-red-600" : "bg-white/10 text-white hover:bg-white/15"
//                         }`}
//                         onClick={toggleCamera}
//                       >
//                         {callSession.isCameraOff ? <VideoOff className="h-5 w-5" /> : <Video className="h-5 w-5" />}
//                       </Button>
//                     ) : null}
//                     <Button
//                       type="button"
...
//                       size="icon"
//                       className={`h-14 w-14 rounded-full border border-white/10 ${
//                         callSession.isSpeakerOn ? "bg-white/10 text-white hover:bg-white/15" : "bg-slate-800 text-slate-300 hover:bg-slate-700"
//                       }`}
...
//                       variant="ghost"
//                       size="icon"
//                       className="h-14 w-14 rounded-full border border-white/10 bg-white/10 text-white hover:bg-white/15"
//                     >
...
//           type,
//           callType: callMode,
//           signalData, // [ĐÃ SỬA]: Dùng signalData thay cho payload để khớp DTO
//         }),
//       });
//     }
//   };
...
//         };
//         sendCallSignal("ice-candidate", candidateData, targetId, callId, callMode); // [ĐÃ SỬA] Đổi type theo chuẩn DTO
//       }
//     };

//     pc.ontrack = (event) => {
//       console.log("[WebRTC] Đã nhận được luồng Media từ người kia!");
//       remoteStreamRef.current = event.streams[0];
//       if (remoteVideoRef.current) {
//         remoteVideoRef.current.srcObject = event.streams[0];
//       }
...
//       sendCallSignal("offer", offerData, targetId, callId, mode);
      
//     } catch (error: any) {
//       console.error("Loi getUserMedia:", error);
//       toast.error("Khong the truy cap camera va micro.");
//       setCallSession((prev) => prev ? { ...prev, hasMediaPermission: false, error: error.message } : prev);
//       closeCall(true);
//     }
//   };
...
//     try {
//       if (!navigator.mediaDevices?.getUserMedia) { 
//         throw new Error("Khong tim thay thiet bi ghi am/hinh."); 
//       }
...
//       // Thêm nút nhấn "accept" để gửi lại backend nếu cần
//       sendCallSignal("accept", null, currentSession.peerId, currentSession.id, currentSession.mode);

//       if (incomingOfferRef.current) {
//         console.log("[WebRTC] Đã có Offer, tiến hành tạo Answer...");
//         await pc.setRemoteDescription(new RTCSessionDescription(incomingOfferRef.current));
//         const answer = await pc.createAnswer();
//         await pc.setLocalDescription(answer);
        
//         const answerData = { type: answer.type, sdp: answer.sdp };
//         sendCallSignal("answer", answerData, currentSession.peerId, currentSession.id, currentSession.mode);
...
//         iceCandidateQueueRef.current = [];
//       } else {
//         console.warn("[WebRTC] Đã nhấn Nghe nhưng chưa nhận được Offer từ STOMP. Sẽ đợi ở luồng STOMP.");
//         toast.info("Đang chờ tín hiệu luồng từ người gọi...");
//       }
//     } catch (err: any) {
//       console.error("Loi acceptCall:", err);
//       toast.error(`Tu choi cuoc goi. Loi thiet bi: ${err.message}`);
//       rejectCall();
//     }
//   };
...
//   const toggleSpeaker = () => {
//     setCallSession((previous) => previous ? { ...previous, isSpeakerOn: !previous.isSpeakerOn } : previous);
//   };
...
//     const timer = setInterval(() => {
//       setCallSession((previous) => previous && previous.startedAt ? { ...previous, elapsedSeconds: Math.floor((Date.now() - previous.startedAt) / 1000) } : previous);
//     }, 1000);
//     return () => clearInterval(timer);
//   }, [callSession?.status, callSession?.startedAt]);
...
//           const newMessage = safeJSONParse(frame.body);
//           if (!newMessage) return; 

//           const conversationId = toId(newMessage.conversationId);
//           const senderId = toId(newMessage.senderId);
//           const receiverId = toId(newMessage.receiverId);
//           const peerId = getEventPeerId(senderId, receiverId);

//           setConversations((previousConversations) => {
//             const updatedConversations = [...previousConversations];
//             const conversationIndex = updatedConversations.findIndex(
//               (conversation) => (conversationId && toId(conversation.id) === conversationId) || (peerId && getConversationPeerId(conversation) === peerId)
//             );

//             if (conversationIndex > -1) {
//               const updatedConversation = { ...updatedConversations[conversationIndex] };
//               updatedConversation.lastMessage = newMessage.content;
//               updatedConversation.time = new Date(newMessage.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });

//               if (senderId !== currentUserId && !doesEventMatchSelectedChat(conversationId, senderId, receiverId)) {
//                 updatedConversation.unread = (updatedConversation.unread || 0) + 1;
//               }

//               if (toId(updatedConversation.id).startsWith("new_") && conversationId) {
//                 const previousPeerId = getConversationPeerId(updatedConversation);
//                 updatedConversation.id = conversationId;
//                 updatedConversation.targetUserId = updatedConversation.targetUserId || previousPeerId || peerId;
//                 if (selectedChatIdRef.current === `new_${previousPeerId}` || selectedChatIdRef.current === `new_${peerId}`) setSelectedChatId(conversationId);
//               }
...
//               if (previousMessages.some((message) => message.id === newMessage.id)) return previousMessages;
//               const cleanMessages = previousMessages.filter((message) => !(message.id.startsWith("temp_") && message.text === newMessage.content));
//               return [ ...cleanMessages, { id: newMessage.id, senderId: senderId === currentUserId ? "me" : senderId, text: newMessage.content, time: new Date(newMessage.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }];
//             });
//           }
//         });

//         client.subscribe(`/user/${currentUserId}/queue/typing`, (frame) => {
//           const event = safeJSONParse(frame.body);
//           if (!event) return;
//           const isUserTyping = event.isTyping === true || event.typing === true;
//           const conversationKey = toId(event.conversationId);
//           const peerId = getEventPeerId(event.senderId, event.receiverId);
//           setTypingUsers((previous) => {
//             const nextTypingUsers = { ...previous };
//             if (conversationKey) nextTypingUsers[conversationKey] = isUserTyping;
//             if (peerId) nextTypingUsers[`new_${peerId}`] = isUserTyping;
//             return nextTypingUsers;
//           });
//         });
...
//           console.log(`[WebRTC] Server gửi về sự kiện: ${event.type}`, event);

//           const { type, callId, callType, senderId, signalData } = event; // [ĐÃ SỬA]: Đọc signalData thay cho payload
//           const parsedData = safeJSONParse(signalData);
...
//                if (callSessionRef.current?.status === "active" && peerConnectionRef.current) {
//                   console.log("[WebRTC] Offer tới trễ! Đang nối luồng trực tiếp...");
//                   const pc = peerConnectionRef.current;
//                   pc.setRemoteDescription(new RTCSessionDescription(parsedData))
//                     .then(() => pc.createAnswer())
//                     .then((answer) => {
//                        return pc.setLocalDescription(answer).then(() => {
//                           sendCallSignal("answer", { type: answer.type, sdp: answer.sdp }, toId(senderId), callId, callType);
...
//                }
//             } else {
//                console.warn("[WebRTC] CẢNH BÁO: Tín hiệu offer có nhưng signalData bị rỗng.");
//             }
//           } else if (type === "answer") {
//             if (parsedData && peerConnectionRef.current) {
//                 console.log("[WebRTC] Đã nhận Answer, kết nối thành công!");
//                 const answerDesc = new RTCSessionDescription(parsedData);
//                 peerConnectionRef.current.setRemoteDescription(answerDesc).then(() => {
...
//                     peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(parsedData)).catch(console.error);
//                 } else {
//                     iceCandidateQueueRef.current.push(parsedData); 
//                 }
//             }
//           } else if (type === "reject" || type === "end") {
//             toast.info(type === "reject" ? "Cuoc goi bi tu choi" : "Cuoc goi da ket thuc");
...
//         setMessages(formattedMessages);
//         setConversations((previousConversations) =>
//           previousConversations.map((conversation) => conversation.id === selectedChatId ? { ...conversation, unread: 0 } : conversation)
//         );
//       } catch (error) {
//         setMessages([]);
//       } finally {
...
//     const content = messageInput.trim();
//     setMessageInput("");
//     const optimisticMessage: MessageItem = { id: `temp_${Date.now()}`, senderId: "me", text: content, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) };
//     setMessages((previousMessages) => [...previousMessages, optimisticMessage]);
//     setConversations((previousConversations) => {
//       const updatedConversations = [...previousConversations];
//       const conversationIndex = updatedConversations.findIndex((conversation) => conversation.id === selectedChat.id);
//       if (conversationIndex > -1) {
//         const updatedConversation = { ...updatedConversations[conversationIndex], lastMessage: `Ban: ${content}`, time: optimisticMessage.time };
//         updatedConversations.splice(conversationIndex, 1);
//         updatedConversations.unshift(updatedConversation);
//       } else if (selectedChatId?.startsWith("new_")) {
//         updatedConversations.unshift({ ...selectedChat, lastMessage: `Ban: ${content}`, time: optimisticMessage.time, unread: 0 });
//       }
//       return dedupConversations(updatedConversations);
//     });
//     if (stompClientRef.current?.connected) {
//       stompClientRef.current.publish({ destination: "/app/chat.sendMessage", body: JSON.stringify({ conversationId: selectedChat.id.startsWith("new_") ? "" : selectedChat.id, receiverId: getConversationPeerId(selectedChat), content, messageType: "text" }) });
//       stompClientRef.current.publish({ destination: "/app/chat.typing", body: JSON.stringify({ conversationId: selectedChat.id.startsWith("new_") ? "" : selectedChat.id, receiverId: getConversationPeerId(selectedChat), isTyping: false, typing: false }) });
//     }
//   };
...
//     setMessageInput(event.target.value);
//     if (stompClientRef.current?.connected && selectedChat) {
//       const typingPayload = { conversationId: selectedChat.id.startsWith("new_") ? "" : selectedChat.id, receiverId: getConversationPeerId(selectedChat), isTyping: true, typing: true };
//       stompClientRef.current.publish({ destination: "/app/chat.typing", body: JSON.stringify(typingPayload) });
//       if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
//       typingTimeoutRef.current = setTimeout(() => {
//         if (stompClientRef.current?.connected) stompClientRef.current.publish({ destination: "/app/chat.typing", body: JSON.stringify({ ...typingPayload, isTyping: false, typing: false }) });
//       }, 2000);
//     }
//   };
...
//   const displayList: ConversationItem[] = isSearchMode
//     ? searchResults.map((user) => ({ id: `new_${toId(user.id)}`, targetUserId: toId(user.id), name: user.name, avatar: user.avatar, lastMessage: "Nhan de mo cuoc tro chuyen ngay.", time: "", unread: 0, isOnline: true, status: "accepted" }))
//     : conversations.filter((conversation) => activeTab === "inbox" ? conversation.status === "accepted" : conversation.status === "pending");
...
//   const selectedHighlights = selectedChat ? [ { icon: BellRing, title: "Do uu tien", value: selectedChat.unread ? "Can xu ly" : "On dinh" }, { icon: Clock3, title: "Lan trao doi", value: selectedChat.time || "Moi bat dau" }, { icon: Star, title: "Trang thai", value: selectedChat.status === "pending" ? "Cho phe duyet" : "San sang ket noi" } ] : [];
...
//   // [TÍNH NĂNG MỚI]: Hàm nhúng tin nhắn trạng thái cuộc gọi vào khung chat
//   const logCallStatusToChat = (session: CallSession, reason: "ended" | "missed" | "rejected") => {
//     if (!stompClientRef.current?.connected) return;

//     let logText = "";
//     if (reason === "rejected") {
//       logText = session.mode === "video" ? "📹 Đã từ chối cuộc gọi video" : "📞 Đã từ chối cuộc gọi thoại";
//     } else if (reason === "missed" || session.elapsedSeconds === 0) {
//       logText = session.mode === "video" ? "📹 Cuộc gọi video nhỡ" : "📞 Cuộc gọi thoại nhỡ";
//     } else {
//       logText = `📞 Cuộc gọi kết thúc. Thời lượng: ${formatCallDuration(session.elapsedSeconds)}`;
//     }
...
//           sdpMLineIndex: event.candidate.sdpMLineIndex
//         };
//         sendCallSignal("ice-candidate", candidateData, targetId, callId, callMode); 
//       }
//     };
...
//       // Phân loại lý do để gửi tin nhắn lịch sử và bắn STOMP
//       if (isReject) {
//          logCallStatusToChat(currentSession, "rejected");
//          sendCallSignal("reject", null, currentSession.peerId, currentSession.id, currentSession.mode);
//       } else {
//          if (currentSession.status === "incoming") {
//             // Đang đổ chuông mà bấm nút tắt tức là Từ chối
//             logCallStatusToChat(currentSession, "rejected");
//             sendCallSignal("reject", null, currentSession.peerId, currentSession.id, currentSession.mode);
//          } else {
//             // Mình là người gọi nhưng tắt máy, hoặc đang gọi mà tắt
//             logCallStatusToChat(currentSession, currentSession.elapsedSeconds === 0 ? "missed" : "ended");
//             sendCallSignal("end", null, currentSession.peerId, currentSession.id, currentSession.mode);
//          }
//       }
//     }
...
//         iceCandidateQueueRef.current = [];
//       } else {
//         toast.info("Đang chờ mạng kết nối luồng..."); // Thay lỗi đỏ thành chữ màu xanh êm ái
//       }
//     } catch (err: any) {
//       toast.error(`Tu choi cuoc goi. Loi thiet bi: ${err.message}`);
//       rejectCall();
...
//   const rejectCall = () => {
//     closeCall(true, true); // Gửi cờ true để hàm closeCall biết đây là hành động Reject
//   };
...
//           const { type, callId, callType, senderId, signalData } = event;
//           const parsedData = safeJSONParse(signalData);
...
//             }
//           } else if (type === "reject" || type === "end") {
//             // [XỬ LÝ ĐÓNG CỬA SỔ BÊN NGƯỜI NHẬN TÍN HIỆU]
//             const currSess = callSessionRef.current;
//             if (currSess) {
//                toast.info(type === "reject" ? "Đã từ chối cuộc gọi" : "Cuộc gọi đã kết thúc");
//             }
//             closeCall(false); // Dọn dẹp máy tính, false = không gửi lại STOMP end để tránh vòng lặp vô tận
//           }
//         });
//       },
//     });
...
//   const localVideoRef = useRef<HTMLVideoElement>(null);
//   const remoteVideoRef = useRef<HTMLVideoElement>(null);
//   const remoteAudioRef = useRef<HTMLAudioElement>(null); // ✅ Tách ref audio riêng
//   const localStreamRef = useRef<MediaStream | null>(null);
//   const remoteStreamRef = useRef<MediaStream | null>(null);
//   const incomingOfferRef = useRef<RTCSessionDescriptionInit | null>(null);
//   const iceCandidateQueueRef = useRef<RTCIceCandidateInit[]>([]);
...
//     // ✅ Đồng bộ ref ngay để tránh race-condition
//     callSessionRef.current = newSession;
...
//       // ✅ Đồng bộ ref NGAY LẬP TỨC trước khi xử lý signaling
//       const nextSession = { ...currentSession, status: "active" as CallStatus, startedAt: Date.now() };
//       callSessionRef.current = nextSession;
//       setCallSession(nextSession);

//       sendCallSignal("accept", null, currentSession.peerId, currentSession.id, currentSession.mode);

//       if (incomingOfferRef.current) {
//         const offerDesc = new RTCSessionDescription(incomingOfferRef.current);
//         incomingOfferRef.current = null; // ✅ Xóa tránh xử lý trùng
...
//       } else {
//         console.log("⏳ Đang chờ SDP Offer từ người gọi...");
//       }
//     } catch (err: any) {
//       toast.error(`Loi thiet bi khi chap nhan cuoc goi: ${err.message}`);
//       rejectCall();
//     }
//   };
...
//   const toggleSpeaker = () => {
//     setCallSession((prev) => (prev ? { ...prev, isSpeakerOn: !prev.isSpeakerOn } : prev));
//   };
...
//     const timer = setInterval(() => {
//       setCallSession((prev) =>
//         prev && prev.startedAt ? { ...prev, elapsedSeconds: Math.floor((Date.now() - prev.startedAt) / 1000) } : prev
//       );
//     }, 1000);
//     return () => clearInterval(timer);
//   }, [callSession?.status, callSession?.startedAt]);
...
//               (c) => (conversationId && toId(c.id) === conversationId) || (peerId && getConversationPeerId(c) === peerId)
//             );
...
//           const convKey = toId(event.conversationId);
//           const peerId = getEventPeerId(event.senderId, event.receiverId);
//           setTypingUsers((prev) => {
//             const next = { ...prev };
//             if (convKey) next[convKey] = isTyping;
//             if (peerId) next[`new_${peerId}`] = isTyping;
//             return next;
//           });
//         });
...
//               incomingOfferRef.current = parsedData;
//               const pc = peerConnectionRef.current;
//               // ✅ Không phụ thuộc UI state, chỉ kiểm tra PC & remoteDescription
//               if (pc && !pc.remoteDescription) {
//                 const offerDesc = new RTCSessionDescription(parsedData);
//                 incomingOfferRef.current = null;
...
//                     }
//                   })
//                   .catch((err) => console.error("❌ Lỗi xử lý Offer:", err));
//               }
//             }
//           } else if (type === "answer") {
//             if (parsedData && peerConnectionRef.current) {
...
//                 })
//                 .catch(console.error);
//               const nextSession = callSessionRef.current
//                 ? { ...callSessionRef.current, status: "active" as CallStatus, startedAt: Date.now() }
//                 : null;
//               if (nextSession) {
//                 callSessionRef.current = nextSession;
//                 setCallSession(nextSession);
...
//       if (idx > -1) {
//         const conv = { ...updated[idx], lastMessage: `Ban: ${content}`, time: optimistic.time };
//         updated.splice(idx, 1);
//         updated.unshift(conv);
//       } else if (selectedChatId?.startsWith("new_")) {
//         updated.unshift({ ...selectedChat, lastMessage: `Ban: ${content}`, time: optimistic.time, unread: 0 });
//       }
//       return dedupConversations(updated);
//     });
//     if (stompClientRef.current?.connected) {
...
//           stompClientRef.current.publish({
//             destination: "/app/chat.typing",
//             body: JSON.stringify({ ...payload, isTyping: false, typing: false }),
//           });
//         }
//       }, 2000);
//     }
...
//   const selectedHighlights = selectedChat
//     ? [
//         { icon: BellRing, title: "Do uu tien", value: selectedChat.unread ? "Can xu ly" : "On dinh" },
//         { icon: Clock3, title: "Lan trao doi", value: selectedChat.time || "Moi bat dau" },
//         { icon: Star, title: "Trang thai", value: selectedChat.status === "pending" ? "Cho phe duyet" : "San sang ket noi" },
//       ]
//     : [];

//   const insightCards = [
//     { label: "Tin moi", value: totalUnread, accent: "from-orange-500 to-amber-500" },
//     { label: "Dang online", value: onlineCount, accent: "from-orange-400 to-orange-500" },
//     { label: "Tin cho", value: pendingConversations.length, accent: "from-amber-500 to-orange-500" },
//   ];
...
//       <div className="mx-auto flex h-full min-h-0 w-full max-w-[1380px]">
//         <div className="flex h-full min-h-0 w-full overflow-hidden rounded-[32px] border border-orange-100 bg-white shadow-[0_20px_56px_rgba(249,115,22,0.06)]">
//           <aside className={`${selectedChat ? "hidden xl:flex" : "flex"} messenger-sidebar w-full shrink-0 flex-col border-r border-orange-100 bg-white xl:w-[336px]`}>
//             <div className="border-b border-orange-100 px-5 pb-4 pt-5">
//               <div className="mb-5 flex items-center justify-between">
//                 <div>
//                   <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-orange-700">
...
//                 </div>
//                 <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-500 hover:bg-orange-50 hover:text-orange-600">
//                   <MoreHorizontal className="h-5 w-5" />
//                 </Button>
//               </div>
...
//                   <div className="flex items-center justify-between gap-4">
//                     <div className="flex min-w-0 items-center gap-3">
//                       <Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600 xl:hidden" onClick={() => setSelectedChatId(null)}>
//                         <ChevronLeft className="h-5 w-5" />
//                       </Button>
//                       <div className="relative shrink-0">
...
//                     </div>
//                     <div className="flex items-center gap-2">
//                       <Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600" onClick={() => startCall("audio")}>
//                         <Phone className="h-4.5 w-4.5" />
//                       </Button>
//                       <Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600" onClick={() => startCall("video")}>
//                         <Video className="h-4.5 w-4.5" />
//                       </Button>
//                       <Button type="button" variant="ghost" size="icon" className={`h-10 w-10 rounded-full border border-orange-100 bg-white transition-colors ${showInfo ? "text-orange-600 ring-2 ring-orange-200" : "text-slate-600 hover:bg-orange-50 hover:text-orange-600"}`} onClick={() => setShowInfo((p) => !p)}>
//                         <Info className="h-4.5 w-4.5" />
//                       </Button>
//                     </div>
//                   </div>
...
//                           </div>
//                           <form onSubmit={handleSendMessage} className="flex items-end gap-2">
//                             <Button type="button" variant="ghost" size="icon" className="h-11 w-11 shrink-0 rounded-2xl border border-orange-100 bg-white text-slate-500 hover:bg-orange-50 hover:text-orange-600">
//                               <ImageIcon className="h-5 w-5" />
//                             </Button>
//                             <div className="messenger-blue-ring flex flex-1 items-end rounded-[28px] border border-orange-100 bg-white px-4 py-2 shadow-sm transition">
...
//                               <button type="button" className="mb-1 ml-3 rounded-full p-1 text-slate-400 transition hover:bg-orange-50 hover:text-orange-600">
//                                 <Smile className="h-5 w-5" />
//                               </button>
//                             </div>
//                             {messageInput.trim() ? (
//                               <Button type="submit" size="icon" className="h-11 w-11 shrink-0 rounded-2xl bg-orange-500 text-white shadow-lg shadow-orange-200 hover:bg-orange-600">
//                                 <Send className="h-4 w-4" />
//                               </Button>
//                             ) : (
//                               <Button type="button" variant="ghost" size="icon" className="h-11 w-11 shrink-0 rounded-2xl border border-orange-100 bg-white text-orange-600 hover:bg-orange-50">
//                                 <ThumbsUp className="h-5 w-5" />
//                               </Button>
//                             )}
//                           </form>
...
//                               <div className="text-xs text-slate-500">Profile nhanh va hanh dong lien quan</div>
//                             </div>
//                             <Button type="button" variant="ghost" size="icon" className="h-9 w-9 rounded-full text-slate-500 hover:bg-slate-100" onClick={() => setShowInfo(false)}>
//                               <X className="h-4.5 w-4.5" />
//                             </Button>
//                           </div>
//                         </div>
...
//                             <div className="mb-3 text-sm font-semibold text-slate-900">Quick actions</div>
//                             <div className="space-y-2">
//                               <Button variant="ghost" className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50">
//                                 <UserIcon className="mr-3 h-4 w-4 text-slate-400" />
//                                 Xem trang ca nhan
//                               </Button>
//                               <Button variant="ghost" className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50" onClick={() => startCall("audio")}>
//                                 <Phone className="mr-3 h-4 w-4 text-slate-400" />
//                                 Bat dau goi thoai
//                               </Button>
//                               <Button variant="ghost" className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50" onClick={() => startCall("video")}>
//                                 <Video className="mr-3 h-4 w-4 text-slate-400" />
//                                 Bat dau video call
//                               </Button>
//                               <Button variant="ghost" className="h-12 w-full justify-start rounded-2xl border border-orange-100 bg-white text-slate-700 hover:bg-orange-50">
...
//                               <Button variant="ghost" className="mt-3 h-11 w-full justify-start rounded-2xl bg-white text-red-600 hover:bg-red-100 hover:text-red-700">
//                                 <X className="mr-3 h-4 w-4" />
//                                 Chan nguoi dung
//                               </Button>
//                             </div>
...
//                     {callSession.mode === "video" ? "Video call" : "Audio call"}
//                   </div>
//                   <Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-white/10 text-white hover:bg-white/15" onClick={() => closeCall(true)}>
//                     <X className="h-4.5 w-4.5" />
//                   </Button>
//                 </div>
...
//                 {callSession.status !== "incoming" && (
//                   <div className="relative z-10 flex flex-wrap items-center justify-center gap-3">
//                     <Button type="button" variant="ghost" size="icon" className={`h-14 w-14 rounded-full border border-white/10 ${callSession.isMuted ? "bg-red-500 text-white hover:bg-red-600" : "bg-white/10 text-white hover:bg-white/15"}`} onClick={toggleMute}>
//                       {callSession.isMuted ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
//                     </Button>
//                     {callSession.mode === "video" && (
//                       <Button type="button" variant="ghost" size="icon" className={`h-14 w-14 rounded-full border border-white/10 ${callSession.isCameraOff ? "bg-red-500 text-white hover:bg-red-600" : "bg-white/10 text-white hover:bg-white/15"}`} onClick={toggleCamera}>
//                         {callSession.isCameraOff ? <VideoOff className="h-5 w-5" /> : <Video className="h-5 w-5" />}
//                       </Button>
//                     )}
//                     <Button type="button" variant="ghost" size="icon" className={`h-14 w-14 rounded-full border border-white/10 ${callSession.isSpeakerOn ? "bg-white/10 text-white hover:bg-white/15" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`} onClick={toggleSpeaker}>
//                       <Volume2 className="h-5 w-5" />
//                     </Button>
//                     <Button type="button" variant="ghost" size="icon" className="h-14 w-14 rounded-full border border-white/10 bg-white/10 text-white hover:bg-white/15">
//                       <MonitorUp className="h-5 w-5" />
//                     </Button>
//                     <Button type="button" size="icon" className="h-14 w-14 rounded-full bg-red-500 text-white hover:bg-red-600" onClick={() => closeCall(true)}>
//                       <PhoneOff className="h-5 w-5" />
//                     </Button>
//                   </div>
//                 )}
...
// // ✅ UPDATED: Thêm reactions & userReactions
// interface MessageItem {
//   id: string;
//   senderId: string;
//   text: string;
...
//       onConnect: () => {
//         const safeJSONParse = (data: any) => {
//           if (!data) return null; if (typeof data === "object") return data; if (data === "undefined" || data === "null") return null;
...
//               updatedUserReactions[reaction.emoji] = true;
//             } else {
//               delete updatedUserReactions[reaction.emoji];
//             }
...
//     client.activate();
//     stompClientRef.current = client;
//     return () => { setTimeout(() => { if (stompClientRef.current?.active) stompClientRef.current.deactivate(); }, 100); };
//   }, [currentUserId]);
...
//                   <div className="flex items-center justify-between gap-4">
//                     <div className="flex min-w-0 items-center gap-3">
//                       <Button type="button" variant="ghost" size="icon" className="h-10 w-10 rounded-full border border-orange-100 bg-white text-slate-600 hover:bg-orange-50 hover:text-orange-600 xl:hidden" onClick={() => setSelectedChatId(null)}><ChevronLeft className="h-5 w-5" /></Button>
//                       <div className="relative shrink-0">
//                         <img src={selectedChatAvatar} alt={selectedChat.name} className="h-12 w-12 rounded-[20px] border border-orange-100 object-cover shadow-sm" />
//                         {selectedChat.isOnline && <span className="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-[3px] border-white bg-emerald-500" />}
//                       </div>
...
export default function MessagesPage({ currentUser }: MessagesPageProps) {
  ...
  useEffect(() => {
    ...
    // ✅ Ngăn tạo trùng client khi StrictMode re-render
    if (stompClientRef.current?.active) {
      stompClientRef.current.deactivate();
    }
