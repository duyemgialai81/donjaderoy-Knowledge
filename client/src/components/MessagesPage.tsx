...
// ✅ CẢI TIẾN: Thêm nhiều STUN và TURN public (OpenRelay) để tăng khả năng kết nối
const iceServers = {
  iceServers: [
    ...
    // TURN public từ OpenRelay (hoạt động nhưng không bảo đảm 100%, có thể thay bằng TURN riêng)
...
export default function MessagesPage({ currentUser }: MessagesPageProps) {
  ...
  const createPeerConnection = (targetId: string, callId: string, callMode: CallMode) => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        sendCallSignal("ice-candidate", { candidate: event.candidate.candidate, sdpMid: event.candidate.sdpMid, sdpMLineIndex: event.candidate.sdpMLineIndex }, targetId, callId, callMode);
      }
    };
    
    pc.ontrack = (event) => {
      console.log("[WebRTC] ontrack called, stream received");
      if (event.streams && event.streams[0]) {
        remoteStreamRef.current = event.streams[0];
        const stream = event.streams[0];
        if (remoteVideoRef.current) remoteVideoRef.current.srcObject = stream;
        if (remoteAudioRef.current) remoteAudioRef.current.srcObject = stream;
        // Force play
        if (remoteVideoRef.current) remoteVideoRef.current.play().catch(e => console.warn("Video play failed:", e));
        if (remoteAudioRef.current) remoteAudioRef.current.play().catch(e => console.warn("Audio play failed:", e));
      }
    };
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      if (pc.iceConnectionState === "failed") {
        toast.error("Kết nối WebRTC thất bại. Có thể do tường lửa hoặc NAT. Thử lại sau.");
        closeCall(true);
      }
      if (pc.iceConnectionState === "connected") {
        toast.success("Kết nối WebRTC thành công!");
      }
    };
  ...
  const startCall = async (mode: CallMode) => {
    ...
    try {
      ...
      toast.error("Khong the truy cap camera va micro. Vui long kiem tra quyen.");
  ...
  const acceptCall = async () => {
    ...
    try {
      ...
      // Set timeout để kiểm tra nếu không có offer sau 5 giây
      acceptCallTimeoutRef.current = setTimeout(() => {
        if (incomingOfferRef.current === null && peerConnectionRef.current && peerConnectionRef.current.signalingState === "stable") {
          toast.error("Không nhận được tín hiệu từ người gọi. Thử lại sau.");
          closeCall(true);
        }
      }, 5000);
      ...
      if (incomingOfferRef.current) {
        ...
        await pc.setRemoteDescription(offerDesc);
        // Xử lý các ICE candidate đã queue
        ...
        client.subscribe(`/user/${currentUserId}/queue/call`, (frame) => {
          ...
          if (type === "start") {
            ...
            if (pc && !pc.remoteDescription) {
              ...
              // Nếu đã có remote description (trường hợp accept trước khi nhận offer)
              console.log("Already have remote description, processing answer later");
            }
          } else if (type === "answer" && parsedData && peerConnectionRef.current) {
            if (acceptCallTimeoutRef.current) clearTimeout(acceptCallTimeoutRef.current);
            const answerDesc = new RTCSessionDescription(parsedData);
  ...
  // Cập nhật video/audio streams khi ref thay đổi hoặc call session thay đổi
  useEffect(() => {
  ...
  }, [callSession?.mode, callSession?.status, localStreamRef.current, remoteStreamRef.current]);
  ...
  // ==================== RENDER (giữ nguyên UI responsive trước đó) ====================
  return (
    <div className="messenger-page h-full min-h-0 bg-white px-3 py-4 md:px-4 md:py-5">
      <div className="mx-auto flex h-full min-h-0 w-full max-w-[1380px]">
        <div className="flex h-full min-h-0 w-full overflow-hidden sm:rounded-[32px] sm:border border-orange-100 bg-white shadow-[0_20px_56px_rgba(249,115,22,0.06)]">
                  ...
                  {callSession.status === "incoming" ? (
                    ...
                    <div className="relative w-full max-w-3xl overflow-hidden rounded-[30px] border border-white/10 bg-slate-900/80 shadow-2xl">
                      ...
                      <div className="flex h-[430px] items-center justify-center">
                        {callSession.hasMediaPermission && !callSession.isCameraOff ? (
                          <>
                            <video ref={remoteVideoRef} autoPlay playsInline onLoadedMetadata={(e) => e.currentTarget.play()} className="h-full w-full object-cover" />
                            <div className="absolute bottom-4 right-4 h-36 w-24 overflow-hidden rounded-2xl border-2 border-white/20 bg-slate-800 shadow-xl">
                              <video ref={localVideoRef} autoPlay muted playsInline onLoadedMetadata={(e) => e.currentTarget.play()} className="h-full w-full object-cover" />
                            </div>
                          </>
                        ) : (
                          <div className="flex flex-col items-center justify-center text-slate-200">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <audio ref={remoteAudioRef} autoPlay playsInline onLoadedMetadata={(e) => e.currentTarget.play()} />
                    </div>
                  )}
                </div>
                {callSession.status !== "incoming" && (
